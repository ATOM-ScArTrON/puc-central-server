...
# Navigate to 'central-server' root (two levels up from 'scripts')
...
# Fast validation of required environment variables without assigning discardable outputs
Assert-EnvVar "MASTER_SERVER_SECRET_HEX"
Assert-EnvVar "MISSION_BROADCAST_KEY_HEX"
Assert-EnvVar "DATABASE_URL"
Assert-EnvVar "ADMIN_PASSWORD"
...
if (-not $env:SERVER_BIND)    { $env:SERVER_BIND = "0.0.0.0" } # Corrected default to match docstring
